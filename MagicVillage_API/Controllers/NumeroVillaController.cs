...
namespace MagicVillage_API.Controllers
{
    ...
    public class NumeroVillaController : ControllerBase
    {
        ...
        public async Task<ActionResult<APIReponse>> GetNumeroVillas()
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                _response.ErrrMessager = new List<string>() { ex.ToString() };
        ...
        public async Task<ActionResult<APIReponse>> GetNumeroVilla(int id)
        {
            try
            {
                if (id == 0)
                {
                    _logger.LogError("Error al traer Numero de Villa con el Id: " + id);
        ...
        public async Task<ActionResult<APIReponse>> CrearNumeroVilla([FromBody] NumeroVillaCreateDto createDto)
        {
            try
            {
                ...
                {
                    ModelState.AddModelError("Numeroxiste", "El número de La Villa ya existe!");
                    return BadRequest(ModelState);
                }
        ...
        public async Task<IActionResult> DeleteNumeroVilla(int id)
        {
            ...
            try
            {
                ...
                var numeroVilla = await _numeroRepo.Obtener(v => v.VillaId == id);
                if (numeroVilla == null)
                {
                    _response.IsExitoso = false;
                    _response.statusCode = HttpStatusCode.NotFound;
